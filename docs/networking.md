...
<details>
<summary>what is SYN Flood attack?</summary>

A **SYN Flood attack** is a type of Denial-of-Service (DoS) attack that exploits the TCP three-way handshake process to overwhelm a target server with a flood of SYN (synchronize) requests. In this attack, the attacker sends a large number of SYN packets to the target server, each appearing to be a legitimate connection request. However, the attacker does not complete the handshake by sending the final ACK (acknowledge) packet. As a result, the server allocates resources for each half-open connection and waits for the ACK that never arrives.

This leads to resource exhaustion on the server, preventing it from handling legitimate connection requests and effectively denying service to legitimate users. SYN Flood attacks can be mitigated using techniques such as SYN cookies, rate limiting, and firewalls.

**Summary**: A SYN Flood attack overwhelms a server with incomplete TCP connection requests, leading to resource exhaustion and denial of service.

</details>

<details>
<summary>Why HTTPS is "slower" than HTTP?</summary>

HTTPS is generally "**slower**" than HTTP due to the additional overhead introduced by the TLS (Transport Layer Security) protocol used to secure HTTPS connections. The main factors contributing to this increased latency include:

1. **TLS Handshake**: Establishing a secure HTTPS connection requires a TLS handshake, which involves multiple round trips between the client and server to negotiate encryption parameters and exchange cryptographic keys. This process adds extra time before data can be transmitted.
2. **Encryption and Decryption**: HTTPS encrypts data before transmission and decrypts it upon receipt. This encryption and decryption process requires computational resources, which can introduce additional latency, especially on devices with limited processing power.
...
<summary>Explain Purge and Versioning</summary>

**Purge**: Purging refers to the process of removing or invalidating cached content from a Content Delivery Network (CDN) or caching system. This is typically done when the original content has been updated or changed, and the cached version is no longer valid. Purging ensures that users receive the most up-to-date content when they request it.
...
<details>
<summary>What is a firewall? How does it work?</summary>
...
<summary>What is the difference between a TCP Health Check and an HTTP Health Check?</summary>

**TCP Health Check**: A TCP health check verifies the availability of a service by attempting to establish a TCP connection to a specified port on the target server. If the connection is successful, the service is considered healthy. TCP health checks are simple and effective for services that use TCP protocols, but they do not provide information about the actual application-level functionality.
...
<details>
<summary>What is a WebSocket? How is it different from HTTP?</summary>
...
<details>
<summary>What is MQTT? How does it work?</summary>
...
When a publisher sends a message to a topic, the broker distributes the message to all subscribers of that topic. MQTT supports different Quality of Service (QoS) levels to ensure reliable message delivery, ranging from "at most once" to "exactly once."

**Summary**: MQTT is a lightweight publish-subscribe messaging protocol used for IoT applications, enabling efficient communication between devices through a central broker.
